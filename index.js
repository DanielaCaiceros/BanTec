...
// Ruta para obtener dinero de un alumno
...
app.post('/add-dinero', async (req, res) => {
  const { id_alumno, cantidad } = req.body;
  try {
    const result = await pool.query(
...
// Ruta para verificar si una respuesta es correcta
app.get('/verificar-respuesta/:id_respuesta', async (req, res) => {
  const idRespuesta = req.params.id_respuesta;

  try {
    const result = await pool.query('SELECT escorrecta FROM respuestas WHERE id_respuesta = $1', [idRespuesta]);
...
// Ruta para obtener preguntas y respuestas de un módulo
...
app.post('/completar-modulo', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM progreso_modulos WHERE id_alumno = $1 AND modulo = $2', [id_alumno, modulo]);
    
    if (result.rows.length > 0) {
      await pool.query('UPDATE progreso_modulos SET completado = TRUE WHERE id_alumno = $1 AND modulo = $2', [id_alumno, modulo]);
    } else {
      await pool.query('INSERT INTO progreso_modulos (id_alumno, modulo, completado) VALUES ($1, $2, TRUE)', [id_alumno, modulo]);
    }
...
app.get('/progreso/:id_alumno', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT modulo FROM progreso_modulos WHERE id_alumno = $1 AND completado = TRUE ORDER BY modulo ASC', [id_alumno]);
